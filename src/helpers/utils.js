...
export const toCurrency = (num = 0, separator = ',', withEnd = true) =>
    num?.toString()?.replace(/(\d)(?=(\d{3})+(?:\.\d+)?$)/g, '$1' + separator) + (withEnd ? ' đ' : '');
...
export const roundDecimal = (num, places = 2) => +(Math.round(num + 'e+' + places) + 'e-' + places);
...
export const deepEqualCompare = (obj1, obj2) => JSON.stringify(obj1) === JSON.stringify(obj2);
