...
export const createMenuStateSlice = (set, get) => ({
  ...
  menuStateActions: {
    ...
    addBillItem: billItem =>
      set(state => {
        ...
        if (index >= 0) {
          ...
          state.menuState.selectedBillItem =
            state.menuState.orderDetails[index];
        }
    ...
    deleteBillItem: (billItem, removeAll = false, removeOnEqual0 = true) =>
      set(state => {
        let index = state.menuState.orderDetails.findIndex(_ =>
          isBillItemEqual(_, billItem, true),
        );
        if (index >= 0) {
          ...
          if (removeAll || (removeOnEqual0 && newCount <= 0)) {
            ...
            if (len > 0) {
              ...
              state.menuState.selectedBillItem =
                state.menuState.orderDetails[nextSelectedIndex];
            } else {
              state.menuState.selectedBillItem = null;
            }
          }
    ...
    addCommentToBillItem: (billItem, comment) =>
      set(state => {
        ...
        if (!comment) {
          delete state.menuState.orderDetails[index].Comment;
        } else {
          state.menuState.orderDetails[index].Comment = comment;
        }
...
export const menuStateSelectors = {
  ...
  setIsOpenFunctionPopup: state =>
    state.menuStateActions.setIsOpenFunctionPopup,
